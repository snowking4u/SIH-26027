...
from app.services.rework import reject_block_plan, revise_block_plan
...
def create_plan(payload: BlockPlanCreate, db: Session = Depends(get_db)):
    if payload.optimization_run_id is not None:
        require(db, OptimizationRun, payload.optimization_run_id, "Optimization run")
    if payload.revises_plan_id is not None:
        original = require(
            db, BlockPlan, payload.revises_plan_id, "Original block plan"
        )
        if original.status != REWORK_REQUIRED:
            raise HTTPException(
                status_code=status.HTTP_409_CONFLICT,
                detail=(
                    "A revised plan may only reference a block plan in "
...
@router.post(
    ...
    response_model=BlockPlanResponse,
    status_code=status.HTTP_201_CREATED,
    summary="Create a revised recommendation after a controller rejection",
    description=(
        "Planning-side rework action. Requires the plan to be in "
        "REWORK_REQUIRED status with a recorded REJECTED decision (its "
        "rejection reason). A new block_plan is created and linked to the "
        "original via revises_plan_id; the original is never deleted or "
        "modified. Other feasible candidates for the same task may be "
        "selected via candidate_block_window_id. The revised plan is "
        "deterministically validated and returns to the controller queue for "
        "a fresh decision."
    ),
)
def revise_plan(
    ...
    try:
        return revise_block_plan(
            db,
            plan_id,
            description=payload.description,
            candidate_block_window_id=payload.candidate_block_window_id,
        )
    except ValueError:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
...
@router.post(
    ...
    description=(
        "Record a human decision on a block plan. APPROVED never "
        "auto-transitions plan status. REJECTED requires a rejection reason "
        "(remarks) and moves the plan into REWORK_REQUIRED so it leaves the "
        "pending queue and is picked up by the planning rework workflow. The "
        "plan and its audit history are preserved."
    ),
)
def create_decision(payload: ControllerDecisionCreate, db: Session = Depends(get_db)):
    plan = require(db, BlockPlan, payload.block_plan_id, "Block plan")
    if payload.decision == "REJECTED":
        if not payload.remarks or not payload.remarks.strip():
            raise HTTPException(
                status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,
                detail="A rejection reason (remarks) is required when rejecting a block plan",
            )
        reject_block_plan(db, plan)
