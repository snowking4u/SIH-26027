    ...
    if not plan_tasks:
        ...
        results.append(
            _result(
                "PLAN_COMPLETENESS",
                PASSED,
                f"PLAN_TASKS_PRESENT:{len(plan_tasks)}",
            )
        )
...
def store_plan_validations(
    ...
    """Persist deterministic validation results for a block plan (idempotent).

    Any previously stored validation records for the plan are replaced by the
    freshly computed ones, so re-validating never accumulates duplicates.

    Does NOT change the block plan status and does NOT approve the plan.
    """
    db.query(PlanValidation).filter(
        PlanValidation.block_plan_id == block_plan_id
